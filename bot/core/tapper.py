...
class Tapper:
    ...
    def pro_server(self):
        ...
        pro_servers = [server["url"] for server in data["server"]["pro"] if server["status"] == 1]

        self.pro_serv = random.choice(pro_servers) if pro_servers else None
        return self.pro_serv

    async def create_payload(self, http_client: aiohttp.ClientSession, game_id, points, dogs):
        if not self.pro_serv:
            self.pro_server()

        url = f"{self.pro_serv}blum/payload" if self.pro_serv else None
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers) as r:
                ...
                if r_data.get("status") == "success":
                    return r_data['data'].get("payload")
                else:
                    return None
